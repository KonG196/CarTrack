...
## 1.5. Поточний стан (15.07.2026)

**Один реальний користувач** — власник продукту і його VW Golf VII Variant 1.6 TDI (2016, ~238 000 км, пригнаний з Німеччини 2022). Його справжня історія засіяна в dev-базі: 19 записів, 7 сервісних інтервалів, 87 323 грн задокументованих витрат в Україні.

**Реалізовано все, що планувалося** — початкові 3 стадії роадмапу плюс 8 ітерацій, виконаних 14–15.07.2026:

| Ітерація | Зміст |
|---|---|
| MVP | авторизація, гараж, журнал 4 типів, розхід «від повного до повного», інтервали з прогнозом, аналітика, PWA |
| OCR + Telegram | розпізнавання чеків (tesseract ukr), бот: привʼязка, /status, пробіг числом, швидкі витрати, нагадування |
| PDF + прогноз | сервісний PDF-паспорт (ReportLab + DejaVu), прогнозна аналітика з оцінкою вартості майбутніх ТО |
| Повний контроль | сторінка запису `/logbook/:id`, редагування всього, фото до записів, пошук, дублювання, **Alembic**, **щоденний бекап у Telegram** |
| Дані в безпеці | JSON/CSV експорт-імпорт, скидання пароля через Telegram, rate limiting, `updated_at` |
| Бот без тертя | «Виконано» одним тапом (транзакційно), пресети ОСЦПВ/техогляду, заправка текстом, фото чека → OCR, /report, inline-кнопки |
| Щоденна заправка | чіпси АЗС, памʼять ціни, санітарні попередження, л/100км у рядку, категорії витрат, десяткова кома |
| Справжня PWA | PNG-іконки + apple-touch-icon, NetworkFirst-кеш, офлайн-банер, черга записів в IndexedDB |
| Документи і діагностика | **OBD-імпорт Car Scanner** з вердиктами (DPF/форсунки/АКБ), тех-довідник, бібліотека документів, VIN+номер з офлайн-декодером, аналітика АЗС |
| Сімʼя | спільний гараж: ролі owner/editor/viewer, інвайт-лінки, авторство записів |
| Зачистка беклогу | ГБО (тип пального на заправку), графік цін, запас ходу, бюджет місяця, шини, тижневий дайджест |

**Масштаб:** 61 REST-ендпоінт, 16 таблиць, 13 Alembic-ревізій, 12 екранів, ~870 бекенд-тестів і ~300 фронтенд-тестів. Білінгу немає — продукт безкоштовний і працює для одного користувача.
...
### 2.3.15 Скидання пароля через Telegram (`app/services/reset.py`)
...
**Акаунт і доступ**
- `users` — id, email (unique), hashed_password (bcrypt), telegram_chat_id, display_name, digest_enabled, reset_code_hash, reset_code_expires_at, created_at
...
| Група | Ендпоінти |
|---|---|
| Auth | `POST /auth/register`, `POST /auth/token`, `GET|PATCH /auth/me`, `POST /auth/reset/request` (**завжди 202**), `POST /auth/reset/confirm` |
...
| `/reset` | `views/ResetPassword.jsx` | **публічний** (два кроки: email → код + новий пароль) |
...
## 5.5 Відомі обмеження і технічний борг (відвертий список станом на 15.07.2026)

Попередня версія цього документа (14.07) містила 14 пунктів. **Десять з них закрито** ітераціями 14–15.07: бекапи, Alembic, скидання пароля, rate limiting, iOS-іконки, офлайн-черга, слабкі дефолти секретів, відсутність редагування, категорії витрат, CSV-експорт. Лишається таке:

1. **Дані користувачів у одному інстансі без реплікації.** Бекап є (щоденний, ротація 14, доставка в адмінський Telegram-чат), але це не HA: падіння диска між бекапами = втрата до доби. Для hosted-SaaS з чужими даними це головний ризик.
2. **JWT у localStorage** (`frontend/src/api/client.js`). XSS = крадіжка токена. Refresh-токенів немає: після `ACCESS_TOKEN_EXPIRE_MINUTES` користувача розлогінює. httpOnly-cookie не використовуються — свідомий компроміс на користь простоти PWA.
3. **Немає email-верифікації.** Реєстрація приймає будь-яку адресу без підтвердження. Скидання пароля працює **лише через привʼязаний Telegram** — акаунт без бота і з забутим паролем відновити неможливо.
4. **Rate limiting — in-memory, per-process.** Перезапуск обнуляє лічильники; за кількох воркерів ліміти множаться на їх кількість. Довіряє `X-Forwarded-For` беззастережно (коректно за nginx, підробне при прямому доступі).
5. **SQLite dev / PostgreSQL prod.** Різні діалекти, різна поведінка NUMERIC і дат; пошук `q` кейс-фолдить кирилицю лише на Postgres (SQLite LIKE цього не вміє).
6. **Мультивалютності немає.** `total_cost` без валюти: митниця в EUR (інвойс 800 EUR) збережена як 0, тобто 87 323 грн **занижують** реальну вартість володіння.
7. **Сирі OBD-логи не зберігаються** — лише min/max/avg/last + даунсемпл ≤200 точок. Порівняти дві сесії покаже-в-покаже або перепарсити файл із новим мапінгом уже не вийде. Розпізнаються тільки відомі колонки Car Scanner; решта — у `unmapped_columns`.
8. **Глобальної бази специфікацій немає** — тех-довідник редагується вручну, є один пресет (Golf 7 1.6 TDI CXXB).
9. **VIN: контрольна цифра не перевіряється** (ЄС-VIN її не використовують); таблиця WMI орієнтована на український парк — невідомий WMI дає `valid: true, manufacturer: null`.
10. **Немає TLS у compose** (nginx на голому 80, термінацію вішати зовні) і жодного моніторингу/алертів — про падіння в проді дізнатися нізвідки.
11. **Бот — один процес на long polling** без захисту від подвійного запуску: два екземпляри з одним токеном конфліктуватимуть на getUpdates.
12. **CI мінімальний**: без лінтерів, type-check, coverage і e2e; компонентних тестів фронтенду немає (лише утиліти й чисті хелпери). Python у venv (3.14) ≠ CI/Docker (3.12).
13. **Нагадування і бекап-доставка привʼязані до старту процесу** (+60 с, далі кожні 24 год) — після деплою час дрейфує.
14. **Спільний доступ без аудиту дій**: видно автора запису, але не історію змін (хто і коли відредагував).
