...
2. **OCR.space** (`ocr_space.py`, безкоштовно, 25k/міс, `OCREngine=2`) — бере те, що tesseract не подужав: мʼяті фото, слабке світло, щільні таблиці нарядів. Вмикається наявністю `OCR_SPACE_API_KEY` або `OCR_SPACE_USE_DEMO_KEY`.
...
Форма наряду не стандартизована, тому парсер читає **структуру, а не розмітку**: рядок, що щось називає і несе гроші, — це позиція; слово поруч із сумою вирішує, це запчастини, роботи чи рахунок. Дати й відсотки вирізаються перед пошуком грошей (інакше «Акт від 17.03.2023» виставляє рахунок на 2023 грн). Сума береться з **найнижчого** рядка з ключовим словом — бо «Разом без ПДВ / ПДВ / Всього до сплати» друкують саме в такому порядку.
