...
| `completeForm.js` | Форма «Виконано» для інтервалу: дефолти, валідація, payload |
...
| `PartsCard.jsx` / `TripCostCard.jsx` | Картка запчастин (Гараж) / калькулятор поїздки (Аналітика) |
...
| `UI.jsx` | `Button`, `TextField`, `SelectField`, `Card`, `Toggle`, `ErrorMessage` |
