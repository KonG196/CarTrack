...
### 2.3.16 Gemini-fallback для OCR (`app/services/ocr_llm.py`)

tesseract добре читає касові чеки АЗС, але не рукописні наряди СТО і не мʼяті фото. Якщо `GEMINI_API_KEY` заданий і tesseract повернув непереконливий результат — запит іде до Gemini vision. Порожній ключ = фіча вимкнена, застосунок лишається повністю офлайновим на самому tesseract. Модель береться з `GEMINI_MODEL` (дефолт `gemini-flash-latest`: аліас «-latest» навмисний, бо Google ретаїть конкретні версії — `2.5-flash` уже віддає 404 новим акаунтам).
...
| Інше | `POST /ocr/scan`, `POST /vin/decode`, `POST /telegram/link-code`, `GET /telegram/status`, `DELETE /telegram/link`, `GET /health` |
