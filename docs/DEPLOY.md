...
Caddy ставимо **на хост**, не в compose: він слухає 80/443 і проксює на бекенд (8000). Фронт віддає Vercel, тож на сервері його немає — цей хост є суто API.
...
```caddyfile
kapot-tracker.duckdns.org {
    reverse_proxy localhost:8000
}
```

Порт **8000, не 3000**: 3000 слухав фронт-контейнер, і всередині його nginx був `location /api/` на `backend:8000` — тобто API виходив назовні через фронт. Фронта більше немає (`replicas: 0` у `docker-compose.lite.yml`), Caddy іде на бекенд напряму. Якщо колись повертатимеш фронт на сервер — повертай і `:3000` тут, інакше він просто не отримає трафіку.
