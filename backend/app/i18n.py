...
MESSAGES: dict[str, dict[str, str]] = {
    ...
    "bot.h.expenseConfirm": {"en": "Expense: «{title}» — {amount:.2f} UAH\nCar: {label}\nDate: {date}\n\nSave?", "uk": "Витрата: «{title}» — {amount:.2f} грн\nАвто: {label}\nДата: {date}\n\nЗберегти?"},
    "bot.h.expenseSaved": {"en": "Expense saved: «{title}» — {amount:.2f} UAH ({date}).", "uk": "Витрату збережено: «{title}» — {amount:.2f} грн ({date})."},
    ...
    "bot.h.maintenanceHeader": {"en": "Service order for {total:.2f} UAH", "uk": "Наряд СТО на {total:.2f} грн"},
    "bot.h.maintenanceMore": {"en": "…and {count} more", "uk": "…та ще {count}"},
    "bot.h.maintenancePartsLabor": {"en": "Parts: {parts:.2f} UAH · Labour: {labor:.2f} UAH", "uk": "Запчастини: {parts:.2f} грн · Роботи: {labor:.2f} грн"},
    "bot.h.maintenanceSaved": {"en": "Service saved: {total:.2f} UAH, {count} items ({date}), mileage {odometer} km.", "uk": "ТО збережено: {total:.2f} грн, {count} позицій ({date}), пробіг {odometer} км."},
    ...
    "bot.h.ocrPartialTotal": {"en": "I only read the amount: {total:.2f} UAH.\nSend the fuel-up as text, e.g.: заправка 45л {total:.0f}", "uk": "Прочитав лише суму: {total:.2f} грн.\nНадішліть заправку текстом, напр.: заправка 45л {total:.0f}"},
    ...
    "bot.h.refuelLine": {"en": "Fuel-up: {liters:.2f} L × {price:.2f} UAH/L = {total:.2f} UAH", "uk": "Заправка: {liters:.2f} л × {price:.2f} грн/л = {total:.2f} грн"},
    "bot.h.refuelPhotoSuffix": {"en": " Receipt photo attached.", "uk": " Фото чека додано."},
    "bot.h.refuelSaved": {"en": "Fuel-up saved: {liters:.2f} L for {total:.2f} UAH ({date}), mileage {odometer} km.{suffix}", "uk": "Заправку збережено: {liters:.2f} л на {total:.2f} грн ({date}), пробіг {odometer} км.{suffix}"},
    ...
    "bot.h.whichCarExpense": {"en": "Which car should I log the expense «{title}» of {amount:.2f} UAH to?", "uk": "До якого авто записати витрату «{title}» на {amount:.2f} грн?"},
    "bot.h.whichCarMaintenance": {"en": "Which car should I log the service of {total:.2f} UAH to?", "uk": "До якого авто записати ТО на {total:.2f} грн?"},
    ...
    "bot.h.whichCarRefuel": {"en": "Which car should I log the fuel-up of {total:.2f} UAH to?", "uk": "До якого авто записати заправку на {total:.2f} грн?"},
