...
from app.models import LogEntry, ServiceInterval

DEFAULT_AVG_DAILY_KM = 40.0


def compute_avg_daily_km(logs: Sequence[LogEntry]) -> float:
    """Average daily km from the car's log history.

    Uses the first and last log entries by date. Requires at least two
    entries, a span of at least one day and a positive odometer delta,
    otherwise falls back to DEFAULT_AVG_DAILY_KM.
    """
    if len(logs) < 2:
        return DEFAULT_AVG_DAILY_KM
    ...
    if day_span >= 1 and odometer_delta > 0:
...
def compute_interval_status(
    ...
    """Compute the derived status fields of a service interval.

    Returns a dict with due_odometer, due_date, km_left, days_left,
    predicted_due_date, health_pct and status per the API contract.
    """
    if today is None:
        today = dt.date.today()
