...
_PROMPT = """Ти — парсер повідомлень авто-логбука. Користувач пише коротке повідомлення українською про свою машину. Визнач ОДНУ дію і витягни числа. Поверни ЛИШЕ JSON:
{
...
Правила:
- Є літри/паливо/АЗС -> action="refuel" (gas_station: ОККО/WOG/SOCAR/UPG/Amic тощо).
- Будь-яка інша витрата (мийка, штраф, страховка, ремонт, ТО, робота СТО) -> action="expense", title = коротка назва.
- Лише показник пробігу («пробіг 154000», «154 тис км») -> action="odometer".
